...
{
	for (Data3D* data : data_) if (DissolveSys::sameString(name, data->name())) return true;
...
{
	for (Data3D* xyData : data_) if (DissolveSys::sameString(name, xyData->name())) return (*xyData);
